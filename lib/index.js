...
function getLcov(lcov, sourceDir, mapExtension) {
	return getTransformedFiles(lcov, sourceDir, mapExtension).then(function (files) {
		return getOutputLcov(files, sourceDir);
	});
}

function writeLcov(lcov, sourceDir, mapExtension, outputFile) {2
	return getLcov(lcov, sourceDir, mapExtension).then(function (lcov) {
...
function getTransformedFiles(lcov, sourceDir, mapExtension) {
	return getData(lcov, sourceDir, mapExtension).then(function (data) {
		return _.chain(data.lcov).map(function (lcov, key) {
			...
			if (!sourcemap) {
				return passthruLcovMap(lcov, sourceDir);
			}
...
function passthruLcovMap(lcov, sourcemap, sourceDir) {
	var files = {};

	var getFile;
	getFile = function (source) {
		var fn = "." + path.sep + path.relative(sourceDir, lcov.file);
